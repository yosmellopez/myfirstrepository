Ext.define('Registro.controller.AreaSaludControl', {
    extend: 'Ext.app.Controller',
    views: ['areaSalud.GridAreaSalud', 'areaSalud.NuevaAreaSalud', 'areaSalud.ModificarAreaSalud'],
    stores: ['AreasSalud'],
    models: ['AreaSalud'],
    refs: [{
            ref: 'grid',
    ...
    init: function () {
        this.control({
            ...
            'nuevaAreaSalud button[action=insertar]': {
                click: this.insertar
            },
            'gridAreaSalud': {
            ...
            'modificarAreaSalud button[action=modificar]': {
    ...
    insertar: function (buton) {
        ...
        if (basicForm.isValid()) {
            ...
            record = Ext.create('Registro.model.AreaSalud', valores);
            store.add(record);
            win.setLoading('Insertando areaSalud...');
            store.sync({
                success: function () {
                    Ext.Msg.show({
                        title: 'Informaci\u00f3n',
                        msg: 'AreaSalud insertada exitosamente',
                        icon: Ext.Msg.INFO,
                        buttons: Ext.Msg.OK
                    });
                    win.close();
    ...
    editar: function (grid, record) {
        win = Ext.widget('modificarAreaSalud').show(grid);
        ...
        if (bform.isValid()) {
            ...
            store.sync({
                success: function () {
                    Ext.Msg.show({
                        title: 'Informaci\u00f3n',
                        msg: 'AreaSalud guardada exitosamente',
                        icon: Ext.Msg.INFO,
                        buttons: Ext.Msg.OK
                    });
                    win.close();
            ...
            Ext.Msg.show({
                ...
                fn: function (btn) {
                    if (btn === 'ok') {
                        ...
                        np = records.length === 1;
                        grid.setLoading('Eliminando areaSalud' + (n === 1 ? '' : 'es') + '...');
                        store.sync({
                            success: function () {
