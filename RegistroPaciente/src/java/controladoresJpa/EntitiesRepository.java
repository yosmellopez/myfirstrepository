...
public abstract class EntitiesRepository<T, ID> {
    ...
    private List<T> listarTodosRegistros(boolean all, int inicio, int fin) {
        ...
        if (orderBy != null && orderBy.length != 0) {
            ...
            for (String order : orderBy) {
                list.add(ascendente ? cb.asc(from.get(order)) : cb.desc(from.get(order)));
            }
            cq.select(from).orderBy(list);
        } else {
            cq.select(from);
        }
    ...
    private List<T> findWhere(Map<String, Object> h, boolean and) {
        ...
        boolean ponerWhere = true, contieneFecha = false, contieneRango = false, fecha = false, rango = false, isString = false, isLike = false, isMultiple = false, pasoJoin = false;
        ...
        for (String clave : claves) {
            if (clave.contains("orderby")) {
                ...
                if (!contieneClave(clave, lista)) {
                    ...
                    if (!valor.isEmpty()) {
                        where += (ponerWhere ? " WHERE " : "") + (i != 0 ? (and ? " AND " : " OR ") : "") + (isMultiple ? multiple : (isLike ? "UPPER(" : "") + ultimoValor + (isString ? (isLike ? like + ") LIKE '%" + valor.toUpperCase() + "%'" : clave + "='" + valor + "'") : (rango ? clave.substring(6) : clave) + (rango ? between : fecha ? "=:" + clave : "='" + valor + "'")));
                        ponerWhere = false;
                        i++;
                    }
                }
        ...
        consulta += where + " " + (orderby.isEmpty() ? " ORDER BY " + getOrderBy() + " DESC" : orderby);
    ...
    @Transactional()
    public void actualizarEntidad(List<T> elements) {
        for (T t : elements) {
            em.merge(t);
        }
    }
